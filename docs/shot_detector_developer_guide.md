...
It is a **multi-modal** problem by design (`docs/SHOTDETECTION.md`): no single
...
`docs/shot_analyzer_developer_guide.md`.
...
- `clubVertical = |z|` of the fused quaternion rotating the sensor long axis
  (+Y, see `docs/IMU_FRAME_CONTRACT.md`) into the +Z-up world frame.
...
signal from a Kalman-tracked ball detector (`docs/BALL_DETECTOR_DESIGN.md` §8).
...
`docs/SHOTDETECTION.md`; for the design this implements (including the P4
roadmap: latency auto-calibration, optional audio-in-ring, ML for jerk-less
sensor placements) see `docs/SHOT_DETECTION_IMPL.md`. Downstream of
`shotDetected`, see `docs/shot_analyzer_developer_guide.md`.*
