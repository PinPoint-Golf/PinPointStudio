...
[`IMU_REARCHITECTURE.md`](IMU_REARCHITECTURE.md) §3.2 / §5; this is the authoritative
...
| **Sensor body** (`imu_link`) | Right-handed, **+X = device-right, +Y = USB-end (long), +Z = face-normal**. Identity quaternion at the neutral pose. | Device characterization ([`IMU_AXIS_REFERENCE.md`](IMU_AXIS_REFERENCE.md)) |
...
[`IMU_REARCHITECTURE.md`](IMU_REARCHITECTURE.md); the explicit `worldToScene()` helper is
