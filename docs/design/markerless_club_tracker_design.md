...
| **P6 — flip** ✅ 2026-09-10 | the markerless profile is the default: `shaft.seg.enabled` 1, `shaft.snap.*` 1 (45 px / 10°, `skipBlur`, coarse-to-fine 2 px / 1.0°), `shaft.head.projPrior` 1 + `latMaxPx` 30 / `latStepPx` 8; `kShaftStageVersion` 1 → 2; new `shaft.snap.skipAddr` (the snap re-registers onto the leg at address) | **§5.2 met on the unmarked 6-iron** (7 swings, 38 seen marks): θ p50 **1.6°** / p90 **4.5°**, head p50 39 → 21 px, measured-head marks 8 → 15, downswing 100%, through-impact 84.9%, finish 173–191 frames, valid 7/7, +539 ms/swing — `docs/research/data/markerless/p6_flip_gate_20260910.md` |
