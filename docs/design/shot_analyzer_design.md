...
- **Wrist score** *(Wrist)* — *which lead-wrist pattern the golfer's motion most resembles, and how strongly.* It is **diagnostic, not a quality grade**: for **each** recognised pattern — **bowed / neutral / cupped** — it computes how closely the swing's lead-wrist flex/extension, radial/ulnar deviation and forearm rotation resemble that pattern's signature through the swing. There is **no selection of one "correct" corridor and no good/bad axis** — bowed and cupped are both workable. The headline surfaces the **closest-matching pattern and its resemblance strength** ("bowed · 86"), telling the golfer *what their wrist pattern is*. Whether that pattern is costing them — e.g. cupping into impact opening the face — is an **AI-coach-layer judgment** (feedback), never baked into the wrist score. It answers *"what is my wrist pattern, and how clearly do I exhibit it?"*
...
> **One model per session (resolves the two-scorer divergence — Appendix C §2).** The wrist estimand is **per-archetype resemblance + a coach-layer fault read**, so the `WristAssessmentEngine` archetype machinery becomes the wrist **score** (resemblance to each of bowed/neutral/cupped → surface the best + label) while its **findings/faults move up to the AI-coach feedback layer** (open-face/cast/flip are coaching judgments, not the score). The impact-only Gaussian `SwingScorer` — and its one-sided "cupping = fault" band — is **superseded for the Wrist session**: cupping is a *pattern*, not a scored fault. (`SwingScorer`'s banded weighted-geometric mean remains the model for the Swing/GRF *adherence* scores.)

**0a. Wrist-archetype resemblance (how the wrist score is computed).** The wrist score is a resemblance to *each* of three reference patterns, **computed, not selected**:
...
- **Uncertainty (§B.7).** Each `R_p` inherits the per-cell FE error (sensor + ~10–15° FE↔RUD cross-talk + phase-timing jitter × local slope), propagated into an interval on the resemblance; **low confidence widens the interval, never shifts the headline**.
