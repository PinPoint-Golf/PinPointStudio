...
### What the wrist score estimates (estimand)

The Wrist session's headline number is a **per-archetype resemblance diagnostic** (0–100) — **not** a quality grade, **not** a between-golfer percentile, **not** a predicted ball-flight outcome. There is **no definition of a "good" wrist pattern**: bowed and cupped can both be made to work (bowed is mildly preferred to cupped, as some players prefer a draw to a fade), so the score does **not** select one correct corridor or penalise a direction. Instead, for **each** recognised pattern — **bowed / neutral / cupped** — it measures how closely the lead-wrist FE/RUD and forearm rotation resemble that pattern's signature through the swing, then **surfaces the closest-matching pattern and its resemblance strength** ("bowed · 86") to tell the golfer *what their wrist pattern is*. Whether that pattern is costing them (e.g. cupping into impact opening the face) is an **AI-coach-layer judgment**, never part of the wrist score. The score carries an **uncertainty interval** (design §B.7): band σ is coaching tolerance only; sensor / cross-talk / phase-timing error is propagated separately, and low confidence **widens the interval rather than raising the score**. Canonical wording in [`shot_analyzer_design.md`](../design/shot_analyzer_design.md) §B.0.

*Implementation:* the resemblance diagnostic is the `WristAssessmentEngine` archetype machinery (re-cast from "detect one archetype + shift its corridor" to "resemblance to **each** archetype"); its **findings/faults move up to the coach feedback layer**. The impact-only Gaussian `SwingScorer` — and the one-sided "cupping = fault" band in the table below — is **superseded for Wrist** (cupping is a pattern, not a scored fault). Scores are **per session, never aggregated across types**; the extractor still pulls every wrist/arm metric the bound sensors support, and the scorer uses only those that belong to a wrist assessment.
