...
| `stripe_annotate.py` | instrumented-club (retro-band) measurement: saturated blobs → collinear RANSAC near the grip anchor → known-ratio pattern match (clubs.json) → theta + px/mm scale + head extrapolation. Anchor tier (≥5 bands, or 4 tight + dark within-group gap) + temporal rescue; no tracker, misses are absent rows. `--truth-out` writes `<swingDir>/truth.json` (`source: "instrumented"`) — the auto-truth path. Scope: club-up phases where bands bloom into discrete saturated blobs; at address the (ring-lit) return only peaks marginally over saturation and fragments — address coverage needs a band/gap profile-correlation detector, not looser blob gates (adjudicated, see lab tape_20260705) |
| `stripe_fusion.py` | **fusion successor to stripe_annotate for truth generation**: per-frame v1 band matcher + polarity-aware ridge (`E2`) + temporal guards. Emission rules are all adjudication-driven: motion-verified corroboration (matched blobs must travel with the hands — scene junk does not; no lone or static-period locks), impact-gap interp guard, forearm veto. Tiers: `band` (theta+s+head) and `ray` (theta-only). `--selftest` = synthetic gate (26 scenes, hard+easy regimes). Coverage: club-in-motion phases (~45–75% down, ~50% thru); address/holds are follow-up (band pattern not optically discrete there at this exposure) |
